...
function initForm(userId, authToken) {
	...
	axios
		const currentPassword = $(".oldpassword").val().trim();// 获取旧密码并去除空格
		const newPassword = $(".newpassword").val().trim(); // 获取新密码并去除空格
		const rnewpassword = $(".rnewpassword").val().trim();// 获取确认密码并去除空格
